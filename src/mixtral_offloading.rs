...
use candle_nn::{Linear, RmsNorm, linear_no_bias};
use std::sync::Arc;
...
impl SparseMoeBlock {
    fn new(cfg: &Config, this_layer: usize, vb: VarBuilder) -> Result<Self> {
...
impl SparseMoeBlock {
    fn forward(&self, xs: &Tensor, expert_cache: &mut ExpertCache<BlockSparseTop2MLP>) -> Result<Tensor> {
...
impl DecoderLayer {
    fn new(rotary_emb: Arc<RotaryEmbedding>, cfg: &Config, vb: VarBuilder, this_layer: usize) -> Result<Self> {
        let self_attn = Attention::new(rotary_emb, cfg, vb.pp("self_attn"))?;
        let block_sparse_moe = SparseMoeBlock::new(cfg, this_layer, vb.pp("block_sparse_moe"))?;
        let input_layernorm =
...
type ExpertFactory<T> = dyn Fn(usize, usize) -> ModuleFactoryResult<T> + Send + Sync + 'static;
...
impl Model {
    ...
    pub fn forward(&mut self, input_ids: &Tensor, seqlen_offset: usize) -> Result<Tensor> {
        ...
        for (layer_idx, layer) in self.layers.iter_mut().enumerate() {
            ...
            xs = layer.forward(&xs, attention_mask.as_ref(), seqlen_offset, &mut self.expert_cache)?
        }
